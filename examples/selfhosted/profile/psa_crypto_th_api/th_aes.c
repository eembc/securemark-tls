...
)
{
    if (mode == AES_ECB)
    {
        *p_context = (psa_cipher_operation_t *)th_malloc(sizeof(psa_cipher_operation_t));
    }
    else if (mode == AES_CCM)
    {
        *p_context = (psa_aead_operation_t *)th_malloc(sizeof(psa_aead_operation_t));
    }
    else
    {
    ...
    if (mode == AES_ECB)
    {
        ...
        *operation = psa_cipher_operation_init();
        ...
        if (func == AES_ENC) {
            ...
            status = psa_import_key(&attributes, p_key, keylen, &key);
            if (status) return EE_STATUS_ERROR;
            status = psa_cipher_decrypt_setup(operation, key, PSA_ALG_ECB_NO_PADDING);
            if (status) return EE_STATUS_ERROR;
        }
    }
    else if (mode == AES_CCM)
...
)
{
    ...
    if (status) return EE_STATUS_ERROR;
    status = psa_aead_finish(operation, NULL, 0, &length, p_tag, taglen, &length);
    if (status) return EE_STATUS_ERROR;
